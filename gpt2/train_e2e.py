...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="./contrast_LM/transformers/examples/full/full/webnlgfinetune_n_20_act_ca"
        "t_b=6-e=10_d=0.0_u=no_lr=1e-05_w=0.0_s=101_r=n_m=512_earlystop",
        help="",
    )
    parser.add_argument(
        "--matching_objective", type=str, default="kl", help="kl or logits"
    )
    ...
    parser.add_argument(
        "--cache_dir",
        type=str,
        default="./contrast_LM/transformers/examples/control",
        help="cache dir",
    )
    ...
    if args.tuning_mode == "adaptertune":
        ...
        if args.notes is None:
            args.notes = "a={}_b={}".format(
                args.adapter_design, args.adapter_bottleneck
            )
        else:
            args.notes = args.notes + "_a={}_b={}".format(
                args.adapter_design, args.adapter_bottleneck
            )
    ...
    if args.mode == "data2text":
        ...
        if args.notes is not None and "lowdata" in args.notes:
            ...
            TRAIN_FILE = "/juice/u/xlisali/e2e_lowdata/lowdata_{}_{}_train.txt".format(
                temp_seed, temp_size
            )
            TEST_FILE = "/juice/u/xlisali/e2e_lowdata/lowdata_{}_{}_valid.txt".format(
                temp_seed, temp_size
            )
            folder_name = "e2e_lowdata_models_prefixtune/"  # 50, 200

            app_special = (
                " --max_steps {} --eval_steps {} --save_steps -1 "
                "--evaluate_during_training --per_device_eval_batch_size 32 "
                "--warmup_steps {} --lowdata_token {} "
                "--use_lowdata_token {} ".format(
                    args.max_steps,
                    args.eval_steps,
                    args.warmup_steps,
                    args.lowdata_token,
            ...
            args.notes = args.notes + "st={}_ev={}_ws={}_t={}".format(
                args.max_steps, args.eval_steps, args.warmup_steps, args.lowdata_token
            )
        ...
        if args.notes is not None and "datalevels" in args.notes:
            ...
            _, temp_seed, temp_size = args.notes.split("_")
            TRAIN_FILE = (
                "/juice/u/xlisali/e2e_datalevels/datalevels_{}_{}_train.txt".format(
                    temp_seed, temp_size
                )
            )
            TEST_FILE = (
                "/juice/u/xlisali/e2e_datalevels/datalevels_{}_{}_valid.txt".format(
                    temp_seed, temp_size
                )
            )
            ...
            args.notes = args.notes + "ev={}_ws={}_t={}_u={}".format(
                args.eval_steps,
                args.warmup_steps,
                args.lowdata_token,
                args.use_lowdata_token,
        ...
        TRAIN_FILE = "./DART/dart/data/v1.1.1/dart-v1.1.1-full-train.json"
        TEST_FILE = "./DART/dart/data/v1.1.1/dart-v1.1.1-full-dev.json"
        ...
        TEST_FILE = (
            "/u/scr/xlisali/contrast_LM/transformers/examples/seq2seq/cnn_dm/val.source"
        )
        ...
        cnndm_app = (
            " --max_source_length {} --train_max_target_length {} "
            "--val_max_target_length {} --dataloader_num_workers 4 ".format(
                max_source_length,
                max_target_length,
        ...
        TRAIN_FILE = (
            "/u/scr/xlisali/contrast_LM/transformers/examples/seq2seq/xsum/train.source"
        )
        TEST_FILE = (
            "/u/scr/xlisali/contrast_LM/transformers/examples/seq2seq/xsum/val.source"
        )
    ...
    if args.init_shallow == "yes":
        if args.notes is not None:
            args.notes = args.notes + "_s={}_w={}".format(
                args.init_shallow, args.init_shallow_word
            )
        else:
    ...
    if args.dir_name is None:
        Model_FILE = (
            ...
            + "_"
            + "b={}-".format(batch_size)
            + "e={}_".format(args.epoch)
            + "d={}_".format(args.dropout)
            + "u={}_".format(args.use_dropout)
            + "lr={}_".format(args.learning_rate)
            + "w={}_".format(args.weight_decay)
            + "s={}".format(args.seed)
            + "_r={}".format(args.init_random[:1])
    ...
    app = (
        "--optim_prefix {} --preseqlen {} --prefix_mode {} --format_mode {} "
        "--gradient_accumulation_steps {} --learning_rate {} --weight_decay {} --seed {} --disable_tqdm "
        "--mid_dim {} --init_random {} --use_dropout {} --prefix_dropout {} --objective_mode {} ".format(
            args.optim_prefix,
            args.preseqlen,
            args.prefix_mode,
            args.format_mode,
    ...
    if args.tuning_mode == "adaptertune":
        app += " --adapter_design {} --adapter_bottleneck {} ".format(
            args.adapter_design, args.adapter_bottleneck
        )
    ...
    if args.init_shallow == "yes":
        app += " --init_shallow {} --init_shallow_word {} ".format(
            args.init_shallow, args.init_shallow_word
        )
    ...
    if load_prefix_model:
        LOAD_TRAIN_PREFIX = "./contrast_LM/transformers/examples/control/med_topic_gen"
    ...
    # #
        ...
        if args.use_big == "no":
            full_command = "nlprun -a lisa-base-torch -g 1 -n {} -x jagupard4,jagupard5,jagupard6,jagupard7,jagupard8,jagupard28,jagupard29,jagupard11,jagupard12,jagupard10 '{}'".format(
                Model_FILE, COMMANDLINE
            )
            if args.mode == "cnndm":
                full_command = "nlprun -a lisa-base-torch -r 20GB -g 1 -n {} -x jagupard4,jagupard5,jagupard6,jagupard7,jagupard8 '{}'".format(
                    Model_FILE, COMMANDLINE
                )
        elif True:
            full_command = (
                "nlprun -p high -a lisa-base-torch -g 1 -n {} -x jagupard4,jagupard5,jagupard6,jagupard7,jagupard8,"
            ...
            full_command = "nlprun -a lisa-base-torch -m jagupard26 -p high -g 1 -n {} '{}'".format(
                Model_FILE, COMMANDLINE
            )
        print(full_command)
