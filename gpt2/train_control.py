    ...
    def __init__(
        ...
        if True:
            ...
            if self.lowdata and self.lowdata_token is not None:
                ...
                print(
                    "LOW DATA SETTING, UNDER PARAMETRIZATION 1, low_data_init=3, "
            ...
            # DIFFERENT PARAMETRIZATION:
                ...
                low_data_init = 0
                print(
                    "[DOUBLE CHECK]: ABLATION STUDY on no parametrization trick... [shallow]"
                )
                ...
                if self.init_shallow_word is not None:
                    ...
                    init_val = self.get_gold_init(
                        model_gpt2, torch.LongTensor(self.init_shallow_word)
                    )
        ...
        if True:
            ...
            if True:
                if self.parametrize_emb == "MLP":
                    print(
                        "MLP: UNDER PARAMETRIZATION 1 FOR embeddings. With the mid_dim = {}".format(
                            self.mid_dim
                        )
                    )
                    self.input_tokens = torch.arange(self.preseqlen).long()
