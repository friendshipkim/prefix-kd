...
def main():
    ...
    if args.task_mode == "data2text":
        ...
        if QUICK_CHECK:
            prompt_text_lst = [
                "name : Blue Spice | Type : coffee shop | area : city centre {}".format(
                    tokenizer.bos_token
                ),
                "name : Blue Spice | Type : coffee shop | customer rating : 5 out of 5 {}".format(
                    tokenizer.bos_token
                ),
                "name : Blue Spice | Type : pub | food : Chinese | area : city centre | family friendly : no {}".format(
                    tokenizer.bos_token
                ),
                "name : Blue Spice | Type : restaurant | food : Chinese | area : city centre | family friendly : yes | near : Rainbow Vegetarian Café {}".format(
                    tokenizer.bos_token
                ),
                "name : Giraffe | Type : restaurant | food : Fast food | area : riverside | family friendly : no | near : Rainbow Vegetarian Café {}".format(
                    tokenizer.bos_token
                ),
                "name : The Cricketers | Type : coffee shop | customer rating : 1 out of 5 | family friendly : yes | near : Avalon {}".format(
                    tokenizer.bos_token
                ),
                "name : The Cricketers | Type : restaurant | food : Chinese | price : high | customer rating : 1 out of 5 | area : city centre | family friendly : no {}".format(
                    tokenizer.bos_token
                ),
                "name : The Mill | Type : restaurant | food : English | price : moderate | area : riverside | family friendly : yes | near : Raja Indian Cuisine {}".format(
                    tokenizer.bos_token
                ),
            ]
            decode_mode = "beam"
            ...
            if ("lowdata" in args.model_name_or_path) or (
                args.prefixModel_name_or_path is not None
                and "lowdata" in args.prefixModel_name_or_path
            ):
            ...
            curr_dir = os.path.join(
                "/u/scr/xlisali/contrast_LM/transformers/examples/text-generation/",
                args.gen_dir,
        ...
        if args.task_mode == "webnlg":
            ...
            test_path = (
                "/u/scr/xlisali/DART/dart/data/v1.1.1/dart-v1.1.1-full-test.json"
            )
        ...
        if QUICK_CHECK:
            ...
            print(
                "the total length of generation should be {}".format(
                    len(prompt_text_lst)
                )
            )
        ...
        if args.task_mode == "cnndm":
            ...
            prompt_text_dict = read_sum_files(
                test_path, tokenizer, max_source_length, max_target_length
            )
    ...
    for prompt_idx, prompt_text in enumerate(prompt_text_lst):
        ...
        if args.tuning_mode == "prefixtune" or args.tuning_mode == "bothtune":
            if args.task_mode == "data2text":
                ...
                mode = None
                if (
                    "cat2" in args.prefixModel_name_or_path
                    or "cat" in args.prefixModel_name_or_path
                ):
                    mode = "cat"
                elif (
                    "nopeek" in args.prefixModel_name_or_path
                    or "nop" in args.prefixModel_name_or_path
                ):
                    mode = "nopeek"
                elif (
                    "peek" in args.prefixModel_name_or_path
                    or "pee" in args.prefixModel_name_or_path
                ):
                    mode = "peek"
                    ...
                    if args.format_mode == "infix":
                        ...
                        assert False, (
                            "Given that it's in prefix tuning mode, need to specify a valid prefix mode, "
                            "(cat, nopeek, peek)"
                        )
    ...
    if args.task_mode == "data2text":
        ...
        print(out_file_eval, "\n", gold_dir, "\n", curr_dir)
        os.system(
            "python /u/scr/xlisali/e2e-metrics/measure_scores.py "
        ...
        os.system(
            "bash /u/scr/xlisali/DART/dart/evaluation/run_eval_on_webnlg.sh "
        ...
        print(curr_dir)
        print(gold_dir)
        temp_command = "python /u/scr/xlisali/classify-eval/eval.py {} {}".format(
            gold_dir, curr_dir
        )
        print(temp_command)
    ...
    if args.task_mode == "topic" or args.task_mode == "sentiment":
        ...
        temp_command = "python /u/scr/xlisali/attribute-eval/eval.py {} ".format(
            curr_dir
        )
        print(temp_command)
        os.system(temp_command)
